...
using Volumix.NativeInterop;
using Volumix.Platform.Linux;
...
try
{
    ...
    else if (command != "apps")
    {
        ...
        var applicationId = new Volumix.Core.ApplicationId(args[1]);
...
{
    Console.Error.WriteLine("libvolumix_native.so was not found. Run ./scripts/build.sh first.");
    return 1;
}
...
static void PrintUsage()
{
    ...
    Console.Error.WriteLine("  volumixctl apps [--watch]");
    Console.Error.WriteLine("  volumixctl set <canonical-id> <0-100>");
    Console.Error.WriteLine("  volumixctl mute <canonical-id>");
    Console.Error.WriteLine("  volumixctl unmute <canonical-id>");
}
...
static void Print(Volumix.Core.MixerSnapshot snapshot)
{
    ...
    foreach (Volumix.Core.RuntimeApplication application in snapshot.Applications)
    {

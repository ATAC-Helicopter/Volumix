...
`VMX-0016` keeps memory and undefined-behavior checks in the required GitHub workflow instead of relying on an occasional local run.
...
Leak detection is disabled for the mixed .NET, PipeWire, and GLib process because those runtimes retain process-lifetime allocations outside Volumix's ownership. Invalid access, use-after-free, buffer errors, and undefined behavior remain fatal.
